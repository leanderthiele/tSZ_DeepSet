...
ax_loss.scatter(guess_loss, loss, label='reconstruction', s=3+20*(logM-vmin)/(vmax-vmin))
if gauss_loss is not None :
    ax_loss.scatter(guess_loss, np.mean(gauss_loss, axis=1), label='<gaussian>', s=3+20*(logM-vmin)/(vmax-vmin))
    for ii in range(gauss_loss.shape[1]) :
        ax_loss.scatter(guess_loss, gauss_loss[:, ii], label='gaussian' if ii==0 else None, c='black', s=0.1)
