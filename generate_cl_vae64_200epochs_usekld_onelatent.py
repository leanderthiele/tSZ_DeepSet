...
def generate_cl(trial) :
    ...
    lr_vae = trial.suggest_float('lr_vae', 1e-7, 1e-4, log=True)
    out.append('ONE_CYCLE_LR_KWARGS["vae"]["max_lr"]=%.8e'%lr_vae)

    wd_vae = trial.suggest_float('wd_vae', 3e-7, 1e0, log=True)
    ...
    residuals_noise = trial.suggest_float('residuals_noise', 1e-3, 1e0, log=True)
    out.append('RESIDUALS_NOISE=%.8e'%residuals_noise)

    kld_scaling = trial.suggest_float('kld_scaling', 1e-6, 3e-4, log=True)
    out.append('KLD_SCALING=%.8e'%kld_scaling)

    mass_noise = trial.suggest_float('mass_noise', 17.0, 30.0)
    out.append('MASS_NOISE=%.8e'%mass_noise)

    dropout = trial.suggest_float('dropout', 0.0, 0.2)
    out.append('DECODER_DROPOUT=%.8e'%dropout)

    visible_dropout= trial.suggest_float('visible_dropout', 0.0, 0.2)
